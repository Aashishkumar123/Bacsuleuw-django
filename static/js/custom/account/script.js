...
function AgencySearchuser(e,usertype){
        var data = $(`#${e.id}`).val()
        $('#searcheduser').html('')
        $.ajax({
            url: "/basculeadmin/search/user/",
            ...
            success: function(data) {
                ...
                for (let index = 0; index < data.data.length; index++) {
                    ...
                    <tr class="odd">
                        ...
                        <td>
                            <a type="button" onClick="SendPassword('${data.data[index].email}')" class="btn btn-promary btn-sm">Send</a>
                        </td>
                        <td>
                            <a href="/basculeadmin/agency/info/?id=${data.data[index].id}" class="menu-link px-3"> View</a>
                        </td>
                        <td>${data.data[index].date_joined}
                        </td>
...
function Agencypagenumber(id,user){
	$('#searcheduser').html('')
	$.ajax({
		type: "GET",
		url: '/basculeadmin/pagination/',
